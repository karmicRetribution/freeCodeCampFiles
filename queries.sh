...
echo "$($PSQL "SELECT DISTINCT(name) FROM games LEFT JOIN teams ON (games.winner_id = team_id) OR (games.opponent_id = team_id) WHERE round='Eighth-Final' AND year=2014 ORDER BY name")"
